-- migration 264 — crear_nota_credito: parámetro opcional p_devolucion_id
--
-- Contraparte de mig.263: ahora que crear_devolucion ya no genera una NC sola,
-- el botón "Generar Nota de Crédito" del detalle de una Devolución (frontend,
-- commit aparte) reutiliza este mismo camino — el que ya calcula bien el IVA
-- por alícuota y ya encola a AFIP (NuevaNCModal) — en vez de duplicar esa
-- lógica. Se le pasa p_devolucion_id para que, al confirmarse la NC, quede
-- linkeada de vuelta a la Devolución que la originó (devoluciones.nota_credito_id
-- + compensacion='nota_credito'), igual que antes quedaba pero ahora como
-- resultado de una elección explícita, no automática.
--
-- DROP explícito: se agrega un parámetro nuevo — CREATE OR REPLACE con distinta
-- firma crea una sobrecarga ambigua en vez de reemplazar (lección de mig.169).

DROP FUNCTION IF EXISTS public.crear_nota_credito(uuid, uuid, uuid, text, text, jsonb, uuid);
...
REVOKE EXECUTE ON FUNCTION public.crear_nota_credito(uuid, uuid, uuid, text, text, jsonb, uuid, uuid) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.crear_nota_credito(uuid, uuid, uuid, text, text, jsonb, uuid, uuid) TO authenticated;

-- ROLLBACK (comentado): restaurar el body de migration 197 (sin p_devolucion_id).
