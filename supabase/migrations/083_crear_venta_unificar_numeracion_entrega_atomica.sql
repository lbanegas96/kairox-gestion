-- ════════════════════════════════════════════════════════════════════════════
-- migration 083 — Fix race condition en numeración de entregas (ENT-YYYY-NNNN)
-- ════════════════════════════════════════════════════════════════════════════
--
-- BUG REAL (confirmado en producción 24/06/2026):
--   ENT-2026-0042 fue asignado a 2 entregas distintas — una generada desde un
--   Pedido y otra desde el POS directamente. Race condition real, no teórica.
--
-- CAUSA RAÍZ:
--   Hay 2 RPCs que generan números de entrega y usaban funciones distintas:
--
--   1) crear_entrega (camino largo, desde Pedido):
--      ✓ usa obtener_proximo_numero(p_empresa_id, 'entrega')
--      → lee series_numeracion.proximo_numero CON FOR UPDATE (atómico)
--      → incrementa proximo_numero en la misma transacción
--
--   2) crear_venta (POS, entrega implícita):
--      ✗ usaba siguiente_numero_documento(p_empresa_id, 'entregas',
--                                          'numero_entrega', 'ENT')
--      → SELECT COUNT(*) FROM entregas WHERE numero_entrega LIKE 'ENT-2026-%'
--      → cuenta filas SIN lock + RETURN count+1
--
--   Los dos caminos NO compartían el mismo source de numeración:
--    - Pedido → series_numeracion.proximo_numero (atómico)
--    - POS    → COUNT(*) en tabla entregas (no atómico)
--
--   Escenario concreto del bug:
--    - series_numeracion.proximo_numero=43, última entrega real ENT-2026-0042.
--    - Usuario A genera entrega desde Pedido → obtener_proximo_numero
--      devuelve 43, incrementa a 44, inserta ENT-2026-0043.
--    - Usuario B confirma venta POS justo después (antes del commit de A o
--      en simultáneo) → siguiente_numero_documento cuenta entregas = 42 → 43
--      → inserta ENT-2026-0043 también. Colisión.
--
-- FIX:
--   crear_venta llama a obtener_proximo_numero(p_empresa_id, 'entrega') igual
--   que crear_entrega. Ambos caminos comparten el mismo source atómico.
--   El tipo 'entrega' YA está registrado en series_numeracion con prefijo
--   'ENT-' y formato YYYY desde migration 051 — no requiere seed adicional.
--
-- Cambio mínimo: 1 línea de la RPC (el resto de la lógica de crear_venta
-- queda exactamente igual). siguiente_numero_documento NO se dropea porque
-- crear_devolucion todavía la usa para tipo 'devoluciones' (deuda futura:
-- migrar también devoluciones a obtener_proximo_numero).
...
BEGIN
  ...
  IF v_entrega_manual_id IS NOT NULL THEN
    ...
    END LOOP;
  ELSE
    -- FIX 083: usar obtener_proximo_numero (atómico via FOR UPDATE en
    -- series_numeracion) en vez de siguiente_numero_documento (COUNT*
    -- sin lock). Asegura que el camino POS comparta el mismo source que
    -- crear_entrega (camino largo desde Pedido) — sin colisiones.
...
-- ROLLBACK (comentado): volver a usar siguiente_numero_documento.
-- NO RECOMENDADO — reintroduce la race condition que causó la colisión
-- ENT-2026-0042 del 24/06/2026.
