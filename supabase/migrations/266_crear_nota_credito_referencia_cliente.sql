-- migration 266 — crear_nota_credito acepta p_referencia_cliente
--
-- Gap detectado en el mapeo vs SAP ("Nota de crédito de clientes" muestra
-- "N° de referencia" con el número de la factura/PO del cliente). La columna
-- comprobantes.referencia_cliente ya existe (mig.258, usada en Factura) pero
-- NuevaNCModal.jsx no tenía el campo — se agrega acá, mismo patrón que
-- Factura: texto libre, opcional, sin impacto en ningún cálculo.
--
-- Copia fiel del resto de la función (pg_get_functiondef, mig.265).
...
BEGIN
  ...
  INSERT INTO public.comprobantes (empresa_id, tenant_id, numero_venta, fecha, cliente_id, cliente_nombre, total, neto_gravado, iva_discriminado, forma_pago, estado_pago, moneda, tipo_cambio_tasa, tipo, comprobante_origen_id, motivo_nc, referencia_cliente)
  VALUES (p_empresa_id, p_empresa_id, v_numero, now(), p_cliente_id, COALESCE(p_cliente_nombre, 'Consumidor Final'), v_total, v_subtotal_neto, v_total_iva, 'Nota de Crédito', 'pagada', 'ARS', 1, 'nota_credito', p_comprobante_origen_id, p_motivo_nc, NULLIF(p_referencia_cliente, ''))
...
-- ROLLBACK (comentado): restaurar el body de migration 265 (sin p_referencia_cliente).
