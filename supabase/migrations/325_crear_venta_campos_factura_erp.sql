-- migration 325 — crear_venta: agregar tipo_comprobante_afip/punto_venta_id/
-- referencia_cliente, para poder reusarla desde "Facturar Pedido/Entrega" del
-- ERP (NuevaFacturaModal.jsx) en vez de que ese flujo abra el POS.
--
-- CONTEXTO (14/08, Luciano): "Facturar Entrega" y "Facturar Pedido" abren
-- NuevaVentaModal — el carrito del Punto de Venta — en vez del formulario de
-- Factura de Venta del ERP. Mal por diseño (KAIROX es un ERP con un POS
-- incluido, no al revés): una factura del ERP necesita elegir tipo de
-- comprobante (A/B/C), punto de venta y referencia del cliente (PO), datos
-- que NuevaVentaModal ni pide ni sabe qué hacer con ellos.
--
-- DECISIÓN DE DISEÑO: no se escribe una RPC nueva en paralelo. crear_venta ya
-- tiene, probada en producción, TODA la lógica de Document Flow que hace
-- falta para facturar desde un pedido/entrega: saltear el descuento de stock
-- si ya hubo una Entrega manual, tope de sobre-facturación contra
-- pedido_items, vínculo pedidos.comprobante_id/entregas.comprobante_id, COGS
-- (costo_mercaderia_vendida). Reescribir todo eso en una función aparte sería
-- duplicar ~150 líneas de lógica financiera ya validada, con el riesgo real
-- de que las dos copias diverjan con el tiempo. Lo único que le falta a
-- crear_venta para poder facturar por afuera del POS es guardar 3 campos que
-- hoy solo pide NuevaFacturaModal.jsx (tipo de comprobante AFIP, punto de
-- venta, referencia del cliente) — se agregan como parámetros OPCIONALES
-- (DEFAULT NULL) al final de la firma, así ningún call site existente
-- (useConfirmarVenta.js del POS, NuevaVentaModal.jsx del ERP) tiene que
-- cambiar una sola línea.
--
-- Sin este fix, la relevancia fiscal quedaba indefinida en este flujo. Ahora
-- el frontend decide con el mismo criterio unificado de siempre (mig.294):
-- el punto de venta elegido determina si el comprobante va a ARCA o queda
-- interno. El resto del cuerpo de la función es IDÉNTICO al ya desplegado.
--
-- OJO — CREATE OR REPLACE FUNCTION con parámetros nuevos crea una SOBRECARGA,
-- no reemplaza: ver la migration 325b, aplicada a continuación de esta para
-- eliminar la firma vieja de 22 parámetros que quedó duplicada.
--
-- ROLLBACK: CREATE OR REPLACE crear_venta sin los 3 parámetros nuevos ni las
-- 3 columnas agregadas al INSERT de comprobantes (definición anterior a esta
-- migration, sin cambios de comportamiento en el resto del cuerpo).
...
CREATE OR REPLACE FUNCTION public.crear_venta(
  ...
  p_pedido_id uuid,
  p_monto_moneda_original numeric DEFAULT NULL::numeric,
...
-- ════════════════════════════════════════════════════════════════════════════════════════
-- Incorporado acá el 25/09/2026 (auditoría COD-3) desde el archivo `325b_fix_crear_venta_overload_duplicado.sql`.
