-- migration 280 — crear_venta devuelve neto_gravado/iva_discriminado
--
-- CONTEXTO: Luciano pidió cerrar el tema "asientos no discriminan IVA
-- Débito/Crédito Fiscal en cuenta aparte" para Ventas Y Compras (mismo
-- comentario que se dejó anotado al cerrar el fix de Compras esta noche).
-- crear_venta YA calcula v_neto_total/v_iva_total y los guarda en
-- comprobantes.neto_gravado/iva_discriminado — pero nunca los devolvía en el
-- jsonb de retorno, así que NuevaVentaModal.jsx/useConfirmarVenta.js no
-- tenían cómo pasárselos a asientosAutoService.crearAsientoVenta (que ahora
-- sabe discriminar IVA Débito Fiscal si se los pasás). Único cambio: agregar
-- 2 claves al RETURN final. Cero cambios de lógica/side-effects.
--
-- (NuevaFacturaModal.jsx no necesita este cambio: calcula neto/iva del lado
-- del cliente porque no pasa por esta RPC.)
