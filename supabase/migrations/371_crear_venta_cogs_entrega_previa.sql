-- migration 371 — crear_venta reconoce el COGS aunque la mercadería ya
-- haya salido por una Entrega anterior (hallazgo Luciano 29/08, mismo
-- circuito PED-20260829-001 → ENT-2026-0150 → Factura: "no dejemos ningún
-- detalle contable para después").
--
-- Causa raíz: `v_costo_total := v_costo_total + (costo_unitario * cantidad)`
-- y el costo_unitario que se guarda en comprobante_items estaban los dos
-- adentro de `IF v_mueve_stock THEN ... END IF` — la misma condición que
-- decide si esta Factura mueve stock FÍSICO. Cuando el pedido ya tuvo una
-- Entrega manual (Regla 8: el stock se mueve una sola vez, en el evento
-- físico), v_mueve_stock queda en false — correcto para el stock, pero de
-- paso el COGS de esa línea también quedaba en $0 (costo_unitario NULL en
-- comprobante_items, nada sumado a costo_mercaderia_vendida). Revisado
-- entrega_items y crear_entrega/crear_entrega_manual (mig.083/139/254): NO
-- guardan costo_unitario en ningún lado — así que para el circuito
-- "Entrega antes de Factura" el COGS nunca se registraba, en ningún punto
-- del flujo. Confirmado con datos reales: PED-20260829-001 tiene Entrega
-- ENT-2026-0150 ya hecha, y facturarlo hoy hubiera dejado
-- costo_mercaderia_vendida en 0 pese a mover $ reales de mercadería.
--
-- Fix: se separa la pregunta "¿esta línea mueve stock FÍSICO ahora?"
-- (v_mueve_stock, sigue gobernando el UPDATE de productos.stock_actual y el
-- movimientos_inventario) de "¿esta línea reconoce costo de mercadería
-- vendida?" (SIEMPRE que sí — la Factura es el documento que reconoce la
-- venta, haya movido stock ella misma o lo haya movido antes la Entrega).
-- costo_unitario se toma igual que siempre, de productos.costo_compra AL
-- MOMENTO DE FACTURAR — no hay (ni había) un snapshot de costo al momento de
-- la Entrega, así que si costo_compra cambió entre la Entrega y la Factura
-- (por una compra nueva de por medio) el costo usado es el vigente al
-- facturar, exactamente el mismo criterio que ya usa el resto del sistema
-- (crear_venta sin pedido, crear_venta_pendiente_qr) — no es una limitación
-- nueva de este fix.
--
-- Verificado que no rompe nada aguas abajo: crear_nota_credito (mig.288)
-- filtra `ci.costo_unitario IS NOT NULL` al revertir COGS de una devolución
-- con reingresa_stock — antes esto EXCLUÍA (incorrectamente) las líneas de
-- este circuito del reverso de COGS; con costo_unitario ahora siempre
-- poblado, las incluye correctamente. El reingreso físico de stock en una
-- devolución (crear_devolucion) está gobernado por el parámetro
-- p_reingresa_stock, no por costo_unitario — no hay ningún otro lugar del
-- código que lea comprobante_items.costo_unitario como proxy de "movió
-- stock" (confirmado por búsqueda en todas las funciones del schema).
...
BEGIN
  ...
  LOOP
    ...
    IF p_pedido_id IS NOT NULL THEN
      -- mig.369 — matchear por pedido_item_id exacto cuando el llamador lo
      -- manda (mismo criterio que crear_entrega, mig.083/139).
      ...
      IF v_pedido_item_id IS NOT NULL THEN
        ...
        END IF;
      ELSE
        -- Compat: llamador que todavía no manda pedido_item_id, o ítem
        -- agregado a mano durante la facturación sin línea de pedido
        -- asociada — mismo comportamiento de antes (ambiguo si el pedido
        -- tiene 2+ líneas del mismo producto).
    ...
    -- mig.370 — el chequeo de stock disponible (y el UPDATE real) solo
    -- corren si esta línea VA a mover stock ahora.
    ...
    -- mig.371 — el COGS se reconoce SIEMPRE en la Factura (costo_compra
    -- vigente × cantidad), sin importar si el movimiento físico de stock ya
    -- pasó antes en una Entrega. v_mueve_stock sigue gobernando SOLO el
    -- movimiento físico (arriba); esto es la contrapartida contable, que la
    -- Factura reconoce siempre porque es el documento que reconoce la venta.
