-- Item 6 del plan de rediseño (22/08) — Cobro/Pago: campo de referencia por
-- método de pago (N° de operación en transferencia, N° de cupón/autorización
-- en tarjeta) + soporte para el comprobante de pago imprimible del frontend.
--
-- Antes solo existía "descripcion" (nota libre) — sin un campo dedicado, no
-- había forma de buscar/filtrar por referencia bancaria ni de mostrarla en
-- un campo propio del comprobante impreso.
...
CREATE OR REPLACE FUNCTION public.registrar_cobro_cliente(
  ...
  p_imputaciones jsonb DEFAULT NULL::jsonb, p_forma_pago_id uuid DEFAULT NULL::uuid,
...
CREATE OR REPLACE FUNCTION public.registrar_pago_proveedor(
  ...
  p_caja_sesion_id uuid DEFAULT NULL::uuid, p_imputaciones jsonb DEFAULT NULL::jsonb,
...
BEGIN
  ...
  INSERT INTO public.cuenta_corriente_proveedores
    (empresa_id, proveedor_id, tipo, monto, descripcion, user_id, fecha, referencia_pago)
  VALUES
    (p_empresa_id, p_proveedor_id, 'pago', v_monto,
     COALESCE(NULLIF(p_descripcion, ''), 'Pago a proveedor'), p_user_id, v_fecha_ts, NULLIF(p_referencia_pago, ''))
