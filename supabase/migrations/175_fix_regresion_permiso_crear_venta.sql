-- REGRESIÓN DE SEGURIDAD: crear_venta perdió el gate has_module_permission('ventas')
-- que la migration 155 le había agregado. Alguna migración posterior que recreó la
-- función (170 monto_moneda_original, 174 centro_costo_id) partió de una copia vieja
-- del body (pre-155) en vez de leer la definición vigente, y el chequeo de permiso
-- se perdió en silencio — sin este fix, cualquier empleado autenticado (sin permiso
-- de módulo 'ventas') puede llamar crear_venta directo por API y registrar una venta
-- completa saltándose el sistema de permisos granulares.
--
-- Confirmado con una query a pg_proc: de las 16 RPCs gateadas por la mig.155, esta es
-- la ÚNICA que perdió el check — las otras 15 (incluyendo registrar_cobro_cliente y
-- registrar_pago_proveedor, también recreadas por migrations posteriores) lo conservan.
--
-- DROP explícito porque la firma actual (con p_monto_moneda_original y
-- p_centro_costo_id, agregados en 170/174) es distinta a la de la mig.155.
