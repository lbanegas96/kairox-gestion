-- Frente 4 de PLAN_FACTURAR_PEDIDO_5_FRENTES.md — Factura de Reserva.
--
-- Pedido de Luciano: poder facturar un Pedido COMPLETO sin que exista
-- ninguna Entrega todavía, y que el movimiento de stock (la Entrega real)
-- ocurra recién DESPUÉS, por separado (con el botón "Generar Entrega" que
-- ya existe).
--
-- Estado antes de esta migration: no existía ningún camino para facturar sin
-- mover stock desde un Pedido. Si no había una Entrega manual previa,
-- crear_venta SIEMPRE movía stock y SIEMPRE creaba una Entrega "implícita"
-- en el mismo paso (ver bloque `IF v_entrega_manual_id IS NOT NULL ... ELSE`
-- más abajo, migration 325) — no hay forma de facturar "en blanco".
--
-- Fix: parámetro nuevo `p_factura_reserva boolean DEFAULT false` (al final
-- de la firma, como los anteriores — ningún call site existente cambia). El
-- resto del cuerpo es IDÉNTICO a la migration 325, con 3 agregados puntuales:
--
--   1. Guard: Factura de Reserva requiere un pedido (no tiene sentido en una
--      factura standalone — confirmado con Nadia, alcance de este frente) y
--      el pedido no puede tener ya una Entrega manual (sería contradictorio:
--      "reservar" es facturar SIN haber entregado nada todavía).
--   2. Al final del loop de ítems, si es reserva, se fuerza v_mueve_stock a
--      FALSE pase lo que pase (nunca se descuenta stock, sin importar si el
--      ítem está o no atado a un pedido_item).
--   3. En el bloque que decide cómo generar la Entrega: si es reserva, no se
--      genera NINGUNA Entrega (ni implícita, ni se linkea una manual, que en
--      este modo no puede existir por el guard del punto 1). La Entrega real
--      se genera después con "Generar Entrega" (crear_entrega), que ya no
--      tiene ningún check de estado — sólo hacía falta destrabar el botón en
--      el frontend (ver TablaPedidos.jsx/ModalDetallePedido.jsx, mismo commit).
--
-- ROLLBACK: CREATE OR REPLACE crear_venta sin `p_factura_reserva` (definición
-- de la migration 325, sin cambios de comportamiento en el resto del cuerpo).
...
CREATE OR REPLACE FUNCTION public.crear_venta(
  ...
  p_factura_reserva boolean DEFAULT false
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- ════════════════════════════════════════════════════════════════════════════════════════
-- Incorporado acá el 25/09/2026 (auditoría COD-3) desde el archivo `328b_fix_crear_venta_overload_reserva.sql`.
