-- migrations/047_crear_venta_fecha_vencimiento.sql
-- Agrega cálculo automático de comprobantes.fecha_vencimiento a la RPC crear_venta.
--
-- COPIA COMPLETA de la lógica vigente (migration 044 — RPC crítica en producción).
-- Único cambio: SELECT de clientes.dias_credito + cálculo de fecha_vencimiento +
-- SET en el INSERT de comprobantes que ya existía. Nada más se modifica.
--
-- Regla: fecha_vencimiento = p_fecha + dias_credito (en días). Si el cliente no
-- tiene dias_credito configurado (NULL) o la venta no tiene cliente, vence el
-- mismo día (COALESCE a 0). Se calcula siempre, sin importar la forma de pago,
-- para mantener un único criterio consistente en todas las ventas — no es
-- exclusivo de Cuenta Corriente.
--
-- IMPORTANTE: este dato es informativo (cuándo vence según la condición de
-- pago del cliente) y NO modifica el cálculo de antigüedad de deuda existente
-- en CuentaCorrienteSection.fetchAgingData() (que sigue midiendo días desde la
-- creación del comprobante, sin tocar).
...
BEGIN
  ...
  -- ── Vencimiento: dias_credito del cliente al momento de la venta ───────────
