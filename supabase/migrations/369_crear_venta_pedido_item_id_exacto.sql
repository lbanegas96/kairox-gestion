-- migration 369 — crear_venta matchea la línea de pedido exacta al facturar
-- (hallazgo Luciano 29/08, "Facturar Pedido — PED-20260829-001": "en ningún
-- lado estoy facturando por 1000" + error real "Cantidad a facturar (1.000)
-- supera lo disponible... máximo facturable: 0.000").
--
-- Causa raíz confirmada con datos reales: PED-20260829-001 tiene "Batidora
-- Eléctrica" en DOS líneas separadas (pedido_items 9ab4c327…, cantidad=2,
-- precio=$25.000 y 9da4f5ea…, cantidad=1, precio=$20.000 — mismo producto,
-- agregado dos veces a precio distinto, caso legítimo estilo SAP). Al
-- facturar, crear_venta buscaba la línea de pedido SOLO por producto_id
-- (sin desambiguar cuál de las dos), así que:
--   1. Ítem "Batidora x2" → toma UNA de las dos filas al azar (sin ORDER BY),
--      la marca cantidad_facturada=2.
--   2. Ítem "Batidora x1" → vuelve a buscar por producto_id, agarra la MISMA
--      fila (ya en cantidad_facturada=2 de cantidad=2) → máximo facturable
--      2-2=0 → rechaza la línea de 1 unidad aunque el pedido en conjunto
--      tenía 3 unidades pedidas y CERO facturadas.
-- El "1.000" del mensaje de error es la cantidad de esa línea (1, con 3
-- decimales — NUMERIC(12,3)), no "mil" — pero el rechazo en sí es un bug real.
--
-- Mismo patrón ya usado y probado en crear_entrega (mig.083/139): el llamador
-- manda pedido_item_id por ítem cuando lo tiene, y la RPC matchea por id
-- exacto en vez de por producto_id. Frontend actualizado en el mismo commit
-- (NuevaFacturaModal.jsx) para mandarlo siempre que factura desde un Pedido.
-- Fallback a la búsqueda vieja por producto_id se mantiene para no romper
-- otros llamadores (POS, etc.) que todavía no mandan pedido_item_id — sigue
-- teniendo la misma ambigüedad en ese caso, pero no es una regresión.
...
BEGIN
  ...
  LOOP
    ...
    IF p_pedido_id IS NOT NULL THEN
      ...
      -- manda (mismo criterio que crear_entrega, mig.083/139). Ver comentario
      -- de cabecera: un pedido con 2 líneas del MISMO producto rompía el
      -- match por producto_id solo.
