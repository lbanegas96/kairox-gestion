-- migration 257 — movimientos_caja.comprobante_id (trazabilidad venta ↔ caja)
--
-- CONTEXTO: para poder cancelar una Factura y revertir su cobro en caja de
-- forma confiable (RPC cancelar_factura, próxima migration), hace falta poder
-- ubicar exactamente qué fila(s) de movimientos_caja pertenecen a qué
-- comprobante. Hoy esa relación no existe como FK — solo se puede inferir por
-- texto (concepto = 'Venta #123' / 'Factura FAC-123'), que es frágil.
--
-- FIX: columna nullable (no rompe histórico) + se empieza a completar desde
-- ahora en crear_venta (este archivo) y NuevaFacturaModal.jsx (commit aparte).
-- Las filas viejas quedan con comprobante_id NULL — cancelar_factura hace
-- fallback por concepto para esos casos legacy.
...
CREATE INDEX IF NOT EXISTS idx_movimientos_caja_comprobante
  ON public.movimientos_caja(comprobante_id)
  WHERE comprobante_id IS NOT NULL;

-- ─── crear_venta — agrega comprobante_id al insert de movimientos_caja ──────
-- Mismo body de migration 224, solo se agrega comprobante_id: v_comprobante_id
-- en el INSERT de movimientos_caja (paso "Cualquier medio no-CC").
...
-- ROLLBACK (comentado): CREATE OR REPLACE FUNCTION crear_venta con el body de
-- migration 224 (sin comprobante_id en el insert de movimientos_caja), y:
-- ALTER TABLE public.movimientos_caja DROP COLUMN IF EXISTS comprobante_id;
