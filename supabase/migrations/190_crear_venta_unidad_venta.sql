-- Migration 190 — crear_venta: guardar la representación de "venta por pack".
-- Sesión 59 (2026-07-10). Complementa mig.189.
--
-- CAMBIO MÍNIMO Y QUIRÚRGICO: el loop de ítems ahora lee 3 campos opcionales del
-- JSON (unidad_venta_id, cantidad_venta, precio_unidad_venta) y los guarda en
-- comprobante_items — SOLO para mostrar el pack en el comprobante impreso.
--
-- La plata, el stock, el IVA, las entregas, la caja y la CC NO cambian: el frontend
-- sigue mandando `cantidad` (en unidad BASE = cantidad_venta × factor), `subtotal` y
-- `precio_unitario` (por unidad base) igual que siempre. Si un ítem no trae los 3
-- campos nuevos (venta normal, sin pack), quedan NULL y la línea es idéntica a hoy.
--
-- Definición base tomada del pg_get_functiondef VIVO en producción (no de archivo).
...
DECLARE
  ...
  v_usa_cc BOOLEAN;
  -- venta por pack (mig.190) — solo representación, no afecta plata/stock
...
BEGIN
  ...
  LOOP
    ...
    -- venta por pack (opcional) — solo se guarda para mostrar, no altera nada más
