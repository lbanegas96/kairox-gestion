-- migration 370 — crear_venta ya no exige stock disponible cuando la
-- mercadería YA salió por una Entrega manual anterior (hallazgo Luciano
-- 29/08, facturando PED-20260829-001 después de generar ENT-2026-0150:
-- "por qué me da error de stock si esto ya fue entregado? ... en este
-- circuito la factura no mueve stock").
--
-- Causa raíz: el chequeo `IF COALESCE(v_stock_disponible, v_stock_actual) <
-- v_cantidad THEN RAISE EXCEPTION 'Stock insuficiente...'` corría SIEMPRE,
-- sin mirar v_mueve_stock — aunque unas líneas más abajo el UPDATE real de
-- stock (`UPDATE productos SET stock_actual = stock_actual - v_cantidad`) sí
-- estaba correctamente condicionado a `IF v_mueve_stock`. Regla 8
-- (sap-reference): el stock se mueve UNA sola vez, en el evento físico
-- (Entrega), nunca de nuevo en la Factura si ya hubo Entrega — pero acá se
-- estaba validando disponibilidad de stock para un movimiento que la propia
-- función sabía que NO iba a hacer. Con ENT-2026-0150 ya entregada (Batidora
-- Eléctrica: 2+1=3 unidades) y el stock físico actual en 2 (post-entrega,
-- correcto), facturar rechazaba con "Stock insuficiente" un movimiento que
-- ni siquiera iba a tocar el inventario.
--
-- Fix: el SELECT de stock_disponible + el RAISE + el UPDATE + la
-- acumulación de costo_total quedan todos adentro del mismo `IF
-- v_mueve_stock THEN` — se valida y se mueve stock juntos, o ninguno de los
-- dos. Se sigue validando "Producto no encontrado" siempre (no depende de si
-- mueve stock).
--
-- Probado con BEGIN...ROLLBACK contra PED-20260829-001 real: la misma
-- llamada a crear_venta que antes fallaba con "Stock insuficiente" ahora
-- pasa sin error y el stock_actual de Batidora Eléctrica queda intacto en 2.
--
-- Hallazgo relacionado (NO corregido en esta migración, a confirmar aparte):
-- cuando v_mueve_stock=false, comprobante_items.costo_unitario también queda
-- NULL y v_costo_total no suma nada para esa línea — es decir, el COGS de
-- una Factura generada DESPUÉS de una Entrega manual (este mismo circuito
-- ERP) queda en $0 para esas líneas, y ni crear_entrega ni
-- crear_entrega_manual guardan costo_unitario en ningún lado (entrega_items
-- no tiene esa columna). Posible gap de COGS específico de este circuito,
-- distinto del que ya cerró la auditoría de mig.287/288/289 (esa cubría el
-- circuito normal de crear_venta, no Entrega-antes-de-Factura).
...
BEGIN
  ...
  LOOP
    ...
    -- corren si esta línea VA a mover stock. Antes el RAISE de "Stock
    -- insuficiente" corría siempre, incluso cuando v_mueve_stock ya era
    -- false porque la mercadería salió en una Entrega anterior (Regla 8) —
    -- rechazaba facturar algo que ni siquiera iba a tocar el inventario.
