-- migration 372 — crear_venta permite Cuenta Corriente combinada con otros
-- métodos de pago (hallazgo Luciano 29/08, probando el POS: "quise pagar con
-- diferentes medios de pago y al restante dejarlo en cuenta y no me deja, es
-- toda la deuda o nada, no me parece muy bien").
--
-- Causa raíz: `p_es_cc` era un booleano de todo-o-nada — si es true, el
-- `INSERT INTO cuenta_corriente_movimientos` usaba SIEMPRE v_total (el total
-- completo de la venta), sin importar qué traía p_pagos. La exclusividad
-- estaba codeada en 3 capas (useMultipago.js, esta RPC, y el motor de
-- asientos contables) — las 3 se corrigen en el mismo commit.
--
-- Fix acá: el monto que va a Cuenta Corriente ahora se calcula SIEMPRE a
-- partir de p_pagos (sumando las líneas con metodo='Cuenta Corriente' — el
-- frontend ya nunca manda más de una), en vez de asumir el total completo.
-- `p_es_cc=true` sin ninguna línea de CC en p_pagos (el caso de siempre para
-- NuevaFacturaModal.jsx — Facturar Pedido, que manda p_pagos=[] y solo el
-- flag) sigue cayendo al comportamiento de compatibilidad: 100% del total a
-- CC. Si SÍ hay una línea de CC en p_pagos (con cualquier monto, parcial o
-- total), esa es la que manda — el flag p_es_cc se vuelve redundante en ese
-- caso pero no estorba.
--
-- El loop de pagos ya saltaba (`IF metodo IS DISTINCT FROM 'Cuenta
-- Corriente'`) la creación de movimientos_caja para una línea de CC — eso
-- YA estaba preparado para este cambio, no se toca.
--
-- Probado con BEGIN...ROLLBACK contra datos reales de Nalux: venta mixta
-- Efectivo $20.000 + Cuenta Corriente $14.200 sobre un total de $34.200 —
-- el DEBE en cuenta_corriente_movimientos quedó en $14.200 (no $34.200), y
-- el movimiento de caja de Efectivo se registró por $20.000.
...
DECLARE
  ...
  -- mig.372 — monto que va a Cuenta Corriente, derivado de p_pagos (puede ser
  -- parcial); monto_paralelo/tc_paralelo de esa línea puntual, no del total.
...
BEGIN
  ...
  -- mig.372 — Cuenta Corriente combinada: el monto que va a CxC se toma de
  -- p_pagos (línea 'Cuenta Corriente', si existe), no siempre v_total.
  -- p_es_cc=true sin esa línea (llamador viejo, ej. Facturar Pedido) sigue
  -- significando "100% del total a CxC".
