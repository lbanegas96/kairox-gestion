...
**Última actualización:** 2026-06-13 (sesión 7 — Luciano) — Prompt 7: kx-* global + fix Caja. fix CajaSection + NuevaVentaModal: SOLO Efectivo requiere caja abierta (Transferencia/Tarjeta/CC operan sin caja). index.css: clases `kairox-*` legacy ahora usan tokens `kx-*`. Reemplazo global en 57 archivos: `bg-white→bg-kx-surface`, `dark:bg-slate-*→dark:bg-kx-*`, `border-slate-200→border-kx-border`, `text-slate-{800,600,400}→text-kx-{text,text-2,text-3}`. Convención: `kx-*` es el estándar visual de todo el sistema, `kairox-*` son aliases legados. Build ✅ 3142 mód. Deploy a producción.
