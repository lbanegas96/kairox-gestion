...
import { Printer, X, Truck, FileText } from 'lucide-react';

const ComprobantePrintModal = ({ open, onOpenChange, comprobante, items, pagos }) => {
  const printRef = useRef();
  const [modoRemito, setModoRemito] = useState(false);
  ...
  const handlePrint = () => {
    ...
    if (windowPrint) {
      windowPrint.document.write(`
        <html>
          <head>
            <title>Imprimir ${modoRemito ? 'Remito' : 'Comprobante'}</title>
            <style>
  ...
  const showPagosDetalle = !modoRemito && pagos && pagos.length > 1;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md kairox-bg-card kairox-text-primary">
        <DialogHeader>
          <DialogTitle>{modoRemito ? 'Remito de Entrega' : 'Comprobante de Venta'}</DialogTitle>
          <DialogDescription>Vista previa del {modoRemito ? 'remito (sin precios)' : 'recibo'}</DialogDescription>
        </DialogHeader>

        {/* Toggle remito/comprobante */}
        <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
          <button
            onClick={() => setModoRemito(false)}
          ...
          >
            <Truck className="h-3.5 w-3.5" /> Remito sin precios
          </button>
        </div>
        ...
        <div className="bg-white text-black p-6 rounded-md shadow-sm border border-slate-200" ref={printRef}>
          <div className="text-center mb-4 border-b border-dashed border-slate-300 pb-4">
            <h2 className="font-bold text-xl uppercase">
              {modoRemito ? 'Remito de Entrega' : 'Ticket de Venta'}
            </h2>
            <p className="text-sm text-slate-500">
              {modoRemito ? 'Remito de Transporte — Sin Valor Comercial' : 'Comprobante No Válido como Factura'}
            </p>
          </div>
          ...
          <table className="w-full text-xs font-mono mb-4">
            <thead>
              <tr className="border-b border-slate-300">
                ...
                {!modoRemito && <th className="text-right py-1">Precio</th>}
                {!modoRemito && <th className="text-right py-1">Subt</th>}
              </tr>
            </thead>
            <tbody>
              {items.map((item, i) => (
                <tr key={i}>
                  ...
                  {!modoRemito && <td className="text-right py-1">${Number(item.precio_unitario).toFixed(2)}</td>}
                  {!modoRemito && <td className="text-right py-1 font-bold">${Number(item.subtotal).toFixed(2)}</td>}
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {modoRemito && (
            <div className="border-t border-dashed border-slate-300 pt-3 text-center text-xs text-slate-500 space-y-1">
              <p>Cantidad de bultos: ___________</p>
              <p>Recibí conforme: ___________________________</p>
              <p className="mt-2">Aclaración: ___________________________</p>
              <p>Fecha de recepción: ___________</p>
            </div>
          )}

          <div className="mt-6 text-center text-xs text-slate-400">
            {modoRemito ? 'Documento interno — No tiene valor fiscal' : 'Gracias por su compra'}
          </div>
        </div>
        ...
        <DialogFooter className="flex gap-2 sm:justify-between">
          ...
          <Button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-700 text-white">
            <Printer className="w-4 h-4 mr-2" /> Imprimir {modoRemito ? 'Remito' : 'Comprobante'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
