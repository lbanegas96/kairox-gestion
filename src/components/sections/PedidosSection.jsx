...
function PedidosSection({ onNavigate, prefillCotizacion, onPrefillConsumed, navigatePedidoId, onNavigated } = {}) {
  ...
  // handleConfirmarDuplicar/handleSave) — null si no viene de "Duplicar".
  const [duplicadoDeIdPendiente, setDuplicadoDeIdPendiente] = useState(null);
  ...
  const openNew = () => {
    ...
    setDuplicadoDeIdPendiente(null);
  ...
  // silencio — abre el mismo form de "Nuevo Pedido" pre-cargado con los datos
  // del original, en modo creación (`editingPedido = null`), para que el
  // usuario pueda revisar/tocar fecha, ítems, cliente, etc. antes de guardar
  // (mismo comportamiento que "Copiar Desde" en SAP). `duplicado_de_id` queda
  // pendiente en estado y se manda recién en `handleSave()`, cuando el usuario
  // efectivamente confirma el alta. `cotizacion_id` NUNCA se copia — un
  // duplicado no es una copia de cotización, es una copia de otro pedido.
  const handleConfirmarDuplicar = (vincular) => {
    ...
    setDuplicadoDeIdPendiente(vincular ? duplicarTarget.id : null);
