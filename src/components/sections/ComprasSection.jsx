      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="nueva" className="mt-0 space-y-4">
          ...
          <div className="kairox-bg-card border kairox-border p-6 rounded-xl flex flex-col relative min-h-[400px] shadow-sm dark:bg-slate-950 dark:border-slate-800">
            ...
            <div className="relative mb-4 z-20"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><Input ref={searchInputRef} placeholder="Buscar producto por nombre o SKU..." value={productSearch} onChange={e => {setProductSearch(e.target.value); setShowAutocomplete(true);}} onKeyDown={handleSearchKeyDown} onFocus={() => setShowAutocomplete(true)} className="pl-9 focus:border-blue-500 dark:focus:border-[#00D4FF] kairox-input dark:bg-slate-900 dark:border-slate-700 dark:text-white"/>{showAutocomplete && (<div className="absolute top-full left-0 w-full kairox-bg-card border kairox-border rounded-md mt-1 shadow-xl max-h-60 overflow-y-auto dark:bg-slate-950 dark:border-slate-800">{filteredProducts.length === 0 ? (<div className="p-3 text-slate-500 text-sm text-center">No se encontraron productos</div>) : (filteredProducts.slice(0, 30).map(p => {const shortUnit = getShortUnit(p.unidad_medida); return (<div key={p.id} className="p-3 flex justify-between items-center border-b kairox-border last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer transition-colors dark:border-slate-800" onClick={() => addToCart(p)}><div><div className="font-medium kairox-text-primary dark:text-white">{p.nombre}</div><div className="text-xs text-slate-500 dark:text-slate-400">{p.codigo_sku} | {p.unidad_medida || 'Unidad'}</div></div><div className="text-right text-slate-600 dark:text-slate-400 text-xs">Costo Actual: ${p.costo_compra?.toFixed(2)}<div className="text-slate-500 dark:text-slate-500">Stock: {p.stock_actual} {shortUnit}</div></div></div>)}))}</div>)}{showAutocomplete && (<div className="fixed inset-0 z-[-1]" onClick={() => setShowAutocomplete(false)}></div>)}</div>
