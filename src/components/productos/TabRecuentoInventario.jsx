...
import ModalNuevoRecuento from './ModalNuevoRecuento';
import ModalDetalleRecuento from './ModalDetalleRecuento';

function TabRecuentoInventario({ categories = [] }) {
  const { user } = useAuth();
  const qc = useQueryClient();
  const [isNuevoOpen, setIsNuevoOpen] = useState(false);
  const [detalleId, setDetalleId] = useState(null);

  const { data: recuentos = [], isLoading } = useQuery({
    queryKey: RECUENTO_INVENTARIO_KEYS.list(user?.empresa_id),
    queryFn: () => recuentoInventarioService.getAll(user.empresa_id),
    enabled: !!user?.empresa_id,
  });

  const invalidar = () => qc.invalidateQueries({ queryKey: RECUENTO_INVENTARIO_KEYS.list(user?.empresa_id) });

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-slate-500 dark:text-kx-text-2">
          Conteo físico de stock contra el sistema — aplica todas las diferencias de una sola vez.
        </p>
        <Button onClick={() => setIsNuevoOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white">
          <Plus className="h-4 w-4 mr-2" /> Nuevo Recuento
        </Button>
      </div>
      ...
      <div className="rounded-xl border border-kx-border dark:border-kx-border overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-kx-surface-2 dark:bg-slate-900/50 text-xs uppercase text-slate-500 dark:text-kx-text-2">
            <tr>
              <th className="p-4 text-left">N° Recuento</th>
      ...
      <ModalDetalleRecuento
        recuentoId={detalleId}
...
export default TabRecuentoInventario;
