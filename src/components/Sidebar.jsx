    ...
    <>
      ...
      >
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-[#0F172A]/50">
          ...
          <button onClick={() => setIsOpen(false)} className="md:hidden text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
            <X size={20} />
          </button>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 group relative
                  ${isActive
                    ? 'bg-blue-50 text-blue-700 dark:bg-slate-700/60 dark:text-slate-100'
              ...
              >
                <div className={`relative ${!isOpen && 'flex justify-center'}`}>
                  <item.icon
                    size={20}
                    className={`transition-colors duration-200 ${isActive ? 'text-blue-700 dark:text-slate-100' : 'group-hover:text-slate-900 dark:group-hover:text-white'}`}
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-[#0F172A]/30">
           {isOpen ? (
             <div className="flex items-center gap-3 bg-white dark:bg-slate-800/50 rounded-xl p-3 border border-slate-200 dark:border-slate-700/50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-blue-600 dark:text-[#00D4FF] capitalize truncate">{user?.role === 'admin' ? 'Administrador' : 'Staff'}</p>
                </div>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-red-500 dark:text-slate-400 dark:hover:text-red-400" onClick={handleSignOut}>
                  <LogOut size={16} />
                </Button>
             </div>
           ) : (
             <div className="flex justify-center group relative">
               <button onClick={handleSignOut} className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 hover:text-red-500 hover:bg-red-50 dark:text-slate-400 dark:hover:text-white dark:hover:bg-red-500/20 transition-colors">
                 <LogOut size={18} />
               </button>
