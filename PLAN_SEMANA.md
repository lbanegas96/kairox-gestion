...
- - ✅ **RESUELTO:** conciliación bancaria / integración Uala. Nuevo archivo `supabase/tests/sync_uala_to_caja.test.sql`, 5/5 verde. Cubre el trigger `trigger_uala_to_caja` (función `sync_uala_to_caja`, dispara `AFTER INSERT ON movimientos_uala`): con caja abierta genera 1 movimiento de caja egreso ligado a esa sesión; destinatario NULL cae al fallback "Ualá → Desconocido". **Hallazgo documentado (no se tocó, es diseño actual a confirmar con Luciano si hace falta cambiarlo):** si el usuario no tiene ninguna caja_sesion abierta en ese momento (sin ninguna, o con la única existente ya cerrada), el trigger no inserta nada y tampoco lanza error — la transferencia de Ualá queda sin reflejarse en `movimientos_caja`, silenciosamente. Casos 3 y 4 del test versionan ese comportamiento tal cual existe hoy.
...
- **Migrations:** numeración secuencial, la última aplicada es `068`. Antes de crear una nueva: `ls supabase/migrations | tail -5` para confirmar el próximo número libre — si están trabajando los 2 en paralelo, avisarse para no colisionar.
